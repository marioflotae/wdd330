  ...
  addHikeListener() {
    ...
    childrenArray.forEach(child => {
      child.addEventListener('click', e => {
        ...
        main.setCommentingOn(e.currentTarget.dataset.name);
        util.displayCurrent();
      });
    });
  }
...
function renderOneHikeFull(hike) {
  const item = document.createElement('li');
  main.setCommentingOn(`${hike.name}`);
  item.innerHTML = ` 
...
export function updateComments(comments){
  ...
  if(Length){
      ...
      commentContainer.innerHTML = "<li>No Comments Yet</li>";
  }
...
function createCommentElement(comment){
  ...
  header.innerHTML = `${comment.name} - ${comment.date}`;
